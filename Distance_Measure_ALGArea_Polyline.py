...
fc11 = raw_input(str('Please enter the name of your comparison layer input from which results will be extrapolated: '))# This must be of Point type geometry, have the distancestamp field --> ARAN Data
##############################################################################################################################
##############################################################################################################################
##############################################################################################################################
print '\n'
...
feats = [fc1,fc2,fc3,fc4,fc5,fc6,fc7,fc8,fc9,fc10,fc11]
...
for fc in fcs:
    if fc != fcs[-1]:
        try:
            ...
            arcpy.AddField_management(fc,"DIST2METR", "DOUBLE",8,5,12) # store shortest distance to end of line
            arcpy.AddField_management(fc,"CLOSE1X", "DOUBLE",20,10,20) # x1 start of line
            arcpy.AddField_management(fc,"CLOSE1Y", "DOUBLE",20,10,20) # y1 start of line
            arcpy.AddField_management(fc,"CLOSE2X", "DOUBLE",20,10,20) # x2 end of line
            arcpy.AddField_management(fc,"CLOSE2Y", "DOUBLE",20,10,20) # y2 end of line
            arcpy.AddField_management(fc,"DistStamp1", "DOUBLE",20,10,20) # distance stamp in meter for start of line
            arcpy.AddField_management(fc,"DistStamp2", "DOUBLE",20,10,20) # distance stamp in meter for end of line
        except:
            pass
...
for fc in fcs:
    if fc != fcs[-1]:
        ...
        with arcpy.da.UpdateCursor(fc, ["Shape@","OBJECTID","DIST1METR","DIST2METR","CLOSE1X","CLOSE1Y","CLOSE2X","CLOSE2Y","DistStamp1","DistStamp2"]) as update:    
            for row1 in update:
                xy1 = row1[0].firstPoint
                startx = xy1.X  
                starty = xy1.Y  
                xy2 = row1[0].lastPoint
                endx = xy2.X    
                endy = xy2.Y
                ...
                with arcpy.da.SearchCursor(srch, ['Shape@','distancest']) as search: 
                    for row2 in search:
                        ...
                        distance2 = distance_2D(endx,xT,endy,yT) 

                        distanceList1.append([distance1,xT,yT,row2[1]])
                        distanceList2.append([distance2,xT,yT,row2[1]])
                ...
                dist21 = distance_2D(distanceList2[0][1],distanceList2[1][1],distanceList2[0][2],distanceList2[1][2])
                dist22 = distanceList2[0][0] 
                dist23 = distanceList2[1][0] 

                row1[2] = (2*area(dist11,dist12,dist13))/dist11 
                row1[3] = (2*area(dist21,dist22,dist23))/dist21 
              
                row1[4] = x_4(distanceList1[0][1],distanceList1[0][2],distanceList1[1][1],distanceList1[1][2],startx,starty)
                row1[5] = y_4(distanceList1[0][1],distanceList1[0][2],distanceList1[1][1],distanceList1[1][2],startx,starty)
                ration1 = distance_2D(distanceList1[0][1],float(str(row1[4])), 
                                      distanceList1[0][2],float(str(row1[5]))) 
                ratio1 = ration1/dist11 
                if float(str(distanceList1[0][3]))>float(str(distanceList1[1][3])): 
                    ratio1 = 1 - ratio1 
                row1[8] = (ratio1 * (max(distanceList1[0][3],distanceList1[1][3])-min(distanceList1[0][3],distanceList1[1][3]))
                                                        + min(float(str(distanceList1[0][3])),float(str(distanceList1[1][3]))))  
                geom1.append(point(row1[4],row1[5]))
                
                row1[6] = x_4(distanceList2[0][1],distanceList2[0][2],distanceList2[1][1],distanceList2[1][2],endx,endy)
                row1[7] = y_4(distanceList2[0][1],distanceList2[0][2],distanceList2[1][1],distanceList2[1][2],endx,endy)
                ration2 = distance_2D(distanceList2[0][1],float(str(row1[6])), 
                                      distanceList2[0][2],float(str(row1[7]))) 
                ratio2 = ration2/dist21 
                if float(str(distanceList2[0][3]))>float(str(distanceList2[1][3])): 
                    ratio2 = 1 - ratio2 
                row1[9] = (ratio2 * (max(distanceList2[0][3],distanceList2[1][3])-min(distanceList2[0][3],distanceList2[1][3]))
                                                        + min(float(str(distanceList2[0][3])),float(str(distanceList2[1][3])))) 
                geom2.append(point(row1[6],row1[7]))
...
print 'In some newly created files, #1 indicates the start of line & #2 the end of line...'
